...
namespace
{
	...
	void
	asyslogger(int msgLvl,const char *fmt,...)
	{
		std::string log;
		char	buf[1024*4];
		va_list	args;
		...
		(void)ReportEvent(hEvent,           // Event log handle.
				((msgLvl == MSGERRO) ? EVENTLOG_ERROR_TYPE : EVENTLOG_INFORMATION_TYPE),  // Event type.
				0,                    // Event category.
				1,                    // Event identifier.
...
namespace StrUtils
{
	...
	std::string
	SubStr(std::string& myStr, size_t start, size_t end)
	{
		size_t x = start;
		size_t y = end;
		if (x>myStr.size())
	...
	void
	Trim(std::string& myStr, bool bLeft, bool bRight)
	{
		if(myStr.empty())
			return;
...
namespace ArgUtils
{
	...
	///   tokenCmd
	//   Description:
	///   \brief A simple command tokeniser to convert a std::string into a set of parameterised
	///   char * arrays
	//   Parameters:
	///   @param int *argc - number of args
	...
	void
	tokenCmd(int *argc, char ***argv, const std::string *cmd)
	{
		if (cmd && !cmd->empty())
		{
...
namespace FileUtils
{
	...
	bool
	GetTmpFilename(std::string &fileName)
	{
		char lName[1024+1];
		...
		if (ltime < 0)
			ltime = 123456;
		(void)sprintf(lName,"%s%smsn%#08x%#08x.tmp",pTmpDir,DIRSEP,getpid(),ltime);
		fileName = lName;
		return true;
	}
}
...
TestTagFile(void)
{
	return (FileUtils::FileExists(afileName));
}
...
namespace SystemUtils
{
	...
	bool
	runCommand(const std::string &cmdLine, std::string &result)
	{
		...
		{
			std::ofstream outputFile(tmpFile.c_str(),(std::ios::out|std::ios::trunc));
			if (outputFile.bad())
			{
				(void)unlink(tmpFile.c_str());
				return false;
...
#define DNSLOOKUPSERV	"checkip.dyndns.org:80"
	...
	///   GetInetAddrLocalIp
	//   Description:
	///   \brief Get an external facing IP addr for this machine from a remote service
	//   Parameters:
	...
	///
	std::string&
	GetInetAddrLocalIp(std::string &addr)
	{
	...
	std::string&
	GetIpAddr(std::string &hostName, std::string &addr)
	{
		if (hostName.empty())
		{
...
#ifndef _WIN32
		char ciddr[128];
		if (inet_ntop(sin.sin_family, &sin.sin_addr, ciddr, sizeof(ciddr))<0)
			addr="";
		else
			addr = ciddr;
#else
...
namespace DebugUtils
{
	...
	void
	LogMessage(int msgLvl,const char *fmt,...)
	{
		std::string log;
		char	buf[1024*4];
		va_list	args;
...
{	
	...
	void
	MSNParseChatLine(std::string &ptrMessage, std::string &reply, bool peekOnly, bool bTrim)
	{
	...
	int
	MSNGetCookieId(const std::string *msg)
	{
		int cookieInvite;
...
namespace HttpUtils
{
	...
	std::string& 
	UrlEncodeString(const std::string *urlString, std::string &myString)
	{
	...
	UrlEncodeString(const std::string &urlString, std::string &myString)
	{
		...
		#define isvalidUrlChar(c) (isalpha(c) || isdigit(c) || c == 0x005f || c == 0x002d)
		#define HexEncodeChar(str,c) { char tmp[10]; (void)sprintf(tmp,"%%%.2x",c); str.append(tmp); }
		...
		if (!urlString.empty())
		{
			...
			while(str[i] && *str)
			{
				...
				/// If the character is needing encoding, then encode it by coverting it to 
				/// its hex code and adding it to the string 
	...
	std::string& UrlDecodeString(const std::string &urlString, std::string &myString)
	{
		/*
