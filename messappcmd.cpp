...
namespace
{
	...
	///   ProcessACmd
	//   Description:
	///   \brief A simple interactive shell to allow the user to enter commands to talk to
	///   the MSN server
	//   Parameters:
	...
	void ProcessACmd(int argc, char **argv, Msn *cMsn)
	{
		...
		else if (!strcasecmp(argv[0],"CONNECT"))
		{
			...
			if (cMsn->Connect())
			{
				...
				std::cout << "MSN User " << cMsn->GetAlias()->c_str() << " has logged in successfully" << std::endl;
			}
			else
			{
		...
		{
			if (argc < 2)
				std::cout << "Status can be available, busy, idle, brb, away, phone or out-to-lunch" << std::endl;
			else
			{
		...
		else if (!strcasecmp(argv[0], "LIST"))
		{
			if (!cMsn->GetGroups()->empty())
			{
				...
				for(it = cMsn->GetGroups()->begin(); it != cMsn->GetGroups()->end(); it++)
				{
					std::string str = *(it);
			...
			if (!cMsn->GetContacts()->empty())
			{
				...
				std::list<std::string>::iterator it;
				for(it = cMsn->GetContacts()->begin(); it != cMsn->GetContacts()->end(); it++)
				{
					std::string str = *(it);
	...
	void ProcessACmd(int argc, char **argv, Yahoo *cYahoo)
	{
		...
		else if (!strcasecmp(argv[0],"CONNECT"))
		{
			...
			{
				std::cout << "Connect ok" << std::endl;
				std::cout << "Yahoo User " << cYahoo->GetAlias()->c_str() << " has logged in successfully" << std::endl;
			}
			else
			{
				std::cout << "Connect failed" << std::endl;
