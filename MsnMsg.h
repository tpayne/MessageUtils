...
public:
	...
	inline const bool IsText() { return (m_contentType.find("text/plain") != std::string::npos); }
	inline const bool IsInvite() { return (m_contentType.find("text/x-msmsgsinvite") != std::string::npos); }
	...
	inline int const CalcPayLoad() { return m_msgTxt.size() + (m_msg.size()-2); }
