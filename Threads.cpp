...
bool 
Threads::operator==(const Threads &val) const
{	
...
/// Overloading the != operator
bool 
Threads::operator!=(const Threads &other) const
{
	return !(*this == other);
}
...
/// Overloading the = operator
Threads& 
Threads::operator=(const Threads &val)
{
	if (this == &val)
		return *this;
...
#else
	int rc = 0;
	m_ThreadHandle = (HANDLE)_beginthreadex(NULL, 0, m_Callback, m_Param, 0, &m_ThreadId );
	m_Started = true;
	return rc;
#endif
}
