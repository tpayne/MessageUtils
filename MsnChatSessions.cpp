...
MsnChatSessions&
MsnChatSessions::operator=(const MsnChatSessions &val)
{
	if (this == &val)
		return *this;
...
void
MsnChatSessions::FormatChatMsg(const char *ptrMessage, std::string &reply)
{
...
void
MsnChatSessions::FormatChatMsg(const std::string &message, std::string &reply)
{
	...
	if (IsDebug())
		(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] %s", __FILE__, __LINE__,
									 reply.c_str());
...
void
MsnChatSessions::FormatChatMsg(MSNChatMsg &message, std::string &reply)
{
...
int
MsnChatSessions::noMsgs2Process(std::string *ptrMessage, std::string *line2Chk)
{
...
bool
MsnChatSessions::ProcessMsg(std::string &ptrMessage)
{
	std::string message = ptrMessage;
	bool bCode = false;
	std::string line;
	...
	if (IsDebug())
		(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] message='%s'", __FILE__, __LINE__,
					  message.c_str());

	MsnUtils::MSNParseChatLine(message,line,true);
	if (IsDebug())
		(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] %s %s", __FILE__, __LINE__,
									 message.c_str(), line.c_str());
	...
	if (ChatLine.IsChat() && ChatLine.GetMsg()->empty())
	{
		...
		///
		if (IsDebug())
			(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] ChatLog line detected", __FILE__, __LINE__);
		bMess = false;
	}
	else if (ChatLine.IsInvite())
	{
		...
		///
		if (ChatLine.GetMsg()->find("Invitation-Command: ACCEPT") != std::string::npos)
		{
			...
			/// I need to kill myself - any new messages will be handled by a new connection
			Disconnect();
			clear();
			GetThread()->Stop();			
		}
		else if (ChatLine.GetMsg()->find("Invitation-Command: CANCEL") != std::string::npos)
		{
	...
	if (bMess && !line.empty())
	{
		...
		if (IsDebug())
			(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] %s", __FILE__, __LINE__, line.c_str());

		FormatChatMsg(myReply, response);

		if (IsDebug())
			(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] %s", __FILE__, __LINE__, response.c_str());
		...
		if (!bCode)
		{
			...
			if (IsDebug())
				(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] %s", __FILE__, __LINE__, GetError()->c_str());
			ptrMessage = message;
			return bCode;
		}
	}
...
int
MsnChatSessions::DoAChat(std::string *ptrMessage)
{
	std::string message = *ptrMessage;
	...
	while(!message.empty())
	{
		...
		if (IsDebug())
			(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] msgcode = %s '%s'", __FILE__, __LINE__, 
										msgcode.c_str(), message.c_str());
		...
		if (msgcode == "BYE")
		{
			...
			if (IsDebug())
				(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] MSN killed me - Bye!", __FILE__, __LINE__);
			Disconnect();
			clear();
...
bool
MsnChatSessions::FileTransfer(const char *fileName)
{
...
bool
MsnChatSessions::FileTransfer(const std::string &fileName)
{
...
///   FileTransferMsnp8
//   Description:
///   \brief Transfer a file via MSNP8 protocol. This assumes I'm sending the file!
//   Parameters:
...
bool
MsnChatSessions::FileTransferMsnp8(const std::string &fileName)
{
	bool bRet = false;
	...
	payLoad = "Application-Name: File Transfer\r\n"
			  ///
			  /// It looks like the application GUI is fixed with different applications having
	          /// different GUIDs, e.g.
	          /// - Application-GUID {F1B1920C-6A3C-4ce7-B18C-AFAB305FD03D} for Netmeeting
	          /// - Application-GUID {56b994a7-380f-410b-9985-c809d78c1bdc} for remote assistence
	          /// - Application-GUID {02D3C01F-BF30-4825-A83A-DE7AF41648AA} for teleconferencing
	          /// - Application-GUID {2A23868E-B45F-401d-B8B0-1E16B774A5B7} for webcam
	          /// - Application-GUID {5D3E02AB-6190-11d3-BBBB-00C04F795683} for file transfer
	          /// ...and so on...
	          ///
	...
	srand (time(NULL));
	int cookieInvite = (rand() % 20000 + 120); /// random number between 120 and 20120
	std::string istr;
...
bool
MsnChatSessions::ProcessFileRequest(MSNChatMsg &ChatLine)
{	
	bool accepted = false;
	bool bRet = false;
	bool bMatch = false;
	...
	if (IsDebug())
		(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] Processing a file request", __FILE__, __LINE__);
	
	int  cookieInvite = MsnUtils::MSNGetCookieId(ChatLine.GetMsg());
	if (cookieInvite < 0)
	{
		SetError(" - Unable to calculate the cookie identifier for this transfer session ");
		return false;
	}
	...
	if (IsDebug())
		(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] A file request will be %s", 
									 __FILE__, __LINE__, (accepted) ? "processed" : "removed");	
	...
	payLoad += "\r\nAuthCookie: ";
	srand (time(NULL));
	int cookieAuth = (rand() % 20000 + 120); /// random number between 120 and 20120
	...
	if (IsDebug())
		(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] Starting up a listener", __FILE__, __LINE__);
	...
	if (IsDebug())
		(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] Waiting for a connection", __FILE__, __LINE__);
	...
	if (IsDebug())
		(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] Waiting for a message", __FILE__, __LINE__);
	...
	/// Get the user id and auth cookie...
	/// Skip USR
	responses = StrUtils::SubStr(responses,responses.find(" "), responses.length());
	...
	if (IsDebug())
		(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] user = %s, cookie=%s", __FILE__, __LINE__, 
									 message.c_str(),responses.c_str());
	...
	if (IsDebug())
		(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] Opening '%s' for transfer", __FILE__, __LINE__, 
									request.GetFile()->c_str());

	/// Start to transfer the file
	int fileNo = open(request.GetFile()->c_str(), O_BINARY|O_RDONLY);  /// Open file in readonly, binary mode
	...
	char *toTransfer = (char*)calloc((MSNFTPPACKSIZ+3),sizeof(char)); /// Allocate memory for the transfer

	if (IsDebug())
		(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] Transferring %d", __FILE__, __LINE__, 
									 filesz);
	...
	while(fileTransferred<filesz) /// Repeat while something to do
	{
		...
		toTransfer[i++] = (char)(packetsz&0x00FF);          /// Second byte = (packet_size MOD 255)
		toTransfer[i++] = (char)((packetsz&0xff00)>>8);     /// Third byte = (packet_size MOD 65280)

		if (IsDebug())
			(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] Transferring %d of %d", __FILE__, __LINE__, 
										 fileTransferred,filesz);
	...
	if (IsDebug())
		(void)DebugUtils::LogMessage(MSGINFO, "Debug: [%s,%d] Transfer done", __FILE__, __LINE__);
