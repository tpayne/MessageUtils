...
NetworkOpsSSL::NetworkOpsSSL(const std::string *host, const std::string *service)
{
	init();
...
bool
NetworkOpsSSL::initCTX(const std::string *chainFile, const std::string *passwd)
{
...
bool
NetworkOpsSSL::Connect(const std::string *chainFile, const std::string *passwd)
{
...
int
NetworkOpsSSL::SendMsg(void *pczMessage, int iMsgLen)
{
	int writen = 0;
	char *tmp = 0;
...
int
NetworkOpsSSL::ReadMsg(char **ptrMessage)
{
	char *ptr = 0;
	char *tmp = 0;
	int total_read = 0;
	...
	while (bCont)
	{
		...
		else
		{
			...
			std::string errOut = (err == SSL_ERROR_SYSCALL) ? " - SSL_ERROR_SYSCALL detected " :
